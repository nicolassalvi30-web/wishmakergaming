...
import { Search, Star, Shield, Gamepad2, LogOut, Edit, Trash2 } from 'lucide-react';
import './style.css';
  ...
  return (
    <>
      ...
      <main className="article">
        ...
        <h1>{post.title}</h1>
        <p className="seo">{post.seo_description}</p>
        {post.cover_image_url && <img className="cover" src={post.cover_image_url} alt={post.title} />}
        <div className="finalScore">{post.score}<small>/10 — {scoreLabel(post.score)}</small></div>
