...
function getSectionItems(body, heading) {
  ...
  for (let i = start + 1; i < lines.length; i++) {
    ...
    if (!line) continue;
    if (/^(Pros|Cons|Final Verdict|Who Should Play It|Gameplay|Replayability|Graphics \/ Presentation|Presentation|Value|Final Score)$/i.test(line) && items.length) break;
...
function AdminCMS() {
  ...
  function editPost(post) {
    ...
    setForm({
      ...
      multiplayer: post.multiplayer || '',
      steam_deck: post.steam_deck || '',
